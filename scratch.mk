# Code generated by scratch version +devel. DO NOT EDIT.
...
# PROJECT_STRUCTURE_SKU_LIMITER_CLIENT sets client library package for Service from sku-limiter.proto
PROJECT_STRUCTURE_SKU_LIMITER_CLIENT=$(PROJECT_STRUCTURE_CLIENTS)/sku-limiter

# DEPRECATED: Use PROJECT_STRUCTURE_*_IMPLEMENTATION instead
PROJECT_STRUCTURE_IMPLEMENTATIONS=internal/app
# PROJECT_STRUCTURE_SKU_LIMITER_IMPLEMENTATION sets implementation package for Service from sku-limiter.proto
PROJECT_STRUCTURE_SKU_LIMITER_IMPLEMENTATION=$(PROJECT_STRUCTURE_IMPLEMENTATIONS)/sku-limiter
...
export GOPROXY=https://goschool.ozon.dev/infra/athens/
export GOSUMDB=off
...
CI_PROJECT_ID?=sku-limiter
LDFLAGS:=-X 'gitlab.ozon.ru/platform/scratch/internal/app.Name=sku-limiter'\
...
SCRATCH_VERSION:=v1.6.12-goschool
...
.PHONY: .get-scratch
.get-scratch:
	@[ -f go.mod ] || go mod init git.ozon.dev/dlukashov/sku-limiter

	go get gitlab.ozon.ru/platform/scratch@v1.6.12-goschool
...
.deps-pb: install-scratch
	...
	rm -rf vendor.pb
	$(SCRATCH_BIN) vendor.pb -v --proto=./api/sku-limiter.proto
...
.build:
	$(info #Building...)
	$(BUILD_ENVPARMS) go build -ldflags "$(LDFLAGS)" -o $(BIN) ./cmd/sku-limiter
...
.run:
	$(info #Running...)
	$(BUILD_ENVPARMS) go run -ldflags "$(LDFLAGS)" ./cmd/sku-limiter --local-config-enabled
...
REL_PATH_FROM_SKU_LIMITER_TO_ROOT=$(shell echo $(PROJECT_STRUCTURE_SKU_LIMITER_CLIENT) | perl -F/ -lane 'print "../"x scalar(@F)')
.PHONY: .generate
.generate:
	mkdir -p $(PROJECT_STRUCTURE_SKU_LIMITER_CLIENT) && cd $(PROJECT_STRUCTURE_SKU_LIMITER_CLIENT) && protoc --plugin=protoc-gen-goclay=$(LOCAL_BIN)/protoc-gen-goclay \
		--plugin=protoc-gen-gofast=$(LOCAL_BIN)/protoc-gen-gofast \
		-I$(REL_PATH_FROM_SKU_LIMITER_TO_ROOT)api/:$(CURDIR)/vendor.pb \
		--gofast_out=$(PKGMAP),plugins=grpc:. \
		--goclay_out=$(PKGMAP),impl=true,impl_path=$(REL_PATH_FROM_SKU_LIMITER_TO_ROOT)$(PROJECT_STRUCTURE_SKU_LIMITER_IMPLEMENTATION),impl_type_name_tmpl=$(IMPLEMENTATION_TYPE_NAME):. \
		$(REL_PATH_FROM_SKU_LIMITER_TO_ROOT)api/sku-limiter.proto
